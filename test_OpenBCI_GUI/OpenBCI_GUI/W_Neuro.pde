...
FFT[] neuroBuff = new FFT[nchan];    //from the minim library
boolean isNeuroFiltered = true; //yes by default ... this is used in dataProcessing.pde to determine which uV array feeds the FFT calculation
int DEFAULTCHANNEL = 1;

class W_Neuro extends Widget {
    ...
    int xLim = xLimOptions[0];  //maximum value of x axis ... in this case 20 Hz, 40 Hz, 60 Hz, 120 Hz
    ...
    void update(){
        ...
        for (int i = 0; i < fft_points.length; i++) {
            for (int j = 0; j < FFT_indexLim + 2; j++) {  //loop through frequency domain data, and store into points array
                GPoint powerAtBin = new GPoint((1.0*sr/nfft)*j, neuroBuff[i].getBand(j));
    ...
    void draw(){
        ...
        /* This is where I begin adding things to the graph
    ...
    int getMaxFrequencyIndex(){
        // Selects a single channel. TestPoint is used to get the coordinates of each individual point. numPoints is the number of points. 
        //
        // Returns: the position in the GPointsArray for which the point of maximum amplitude occurs. 

        GPointsArray channel = fft_points[DEFAULTCHANNEL];
        GPoint testPoint;
        int numPoints = channel.getNPoints();

        int index = 0;
        double maxAmp = -1.0;

        // Calculates the frequency at which maximum amplitude occurs. 
        for(int i=0; i < numPoints; i++){
            testPoint = channel.get(i);
            if(testPoint.getY() > maxAmp){
                maxAmp = testPoint.getY();
                index = i;
            }
        }
        return index;
    ...
    double calculateSNR(int maxIndex){
        // Selects a single channel. TestPoint is used to get the coordinates of each individual point. numPoints is the number of points. 
        //
        // Returns: The calculated SNR value!
        ...
        for(int i=0; i < numPoints; i++){
            testPoint = channel.get(i);
            if(testPoint.getX() >= 4 && testPoint.getX() <= 15){
                totalVal += testPoint.getY();
            }
        }

        // Returns the value of the highest maximum minus the average value. 
        double maxAmp = channel.get(maxIndex).getY();

        return maxAmp - (totalVal / numPoints);
    }
    

    /*
    void printDirection(int maxIndex, double snr){
        GPointsArray channel = fft_points[DEFAULTCHANNEL];
        double maxFreq = 
...
void UnfiltFilt(int n) {
    if (n==0) {
        ...
        isNeuroFiltered = true;
    } else {
        //have FFT use unfiltered data
        isNeuroFiltered = false;
    }
    //since this function is called by both the BandPower and FFT Widgets the dropdown needs to be updated in both
